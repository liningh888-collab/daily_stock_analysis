...
def get_market_status():
    ...
    try:
        ...
        for code in indexes:
            # 改用yf.download更稳定
            df = yf.download(code, period="60d", progress=False, timeout=10)
            if len(df) < 30:
                continue
...
def calc_technical_indicators(df, mode):
    ...
    rsv = (close - low9) / hl_range * 100
    k, d = rsv.ewm(span=3, adjust=False).mean(), rsv.ewm(span=3, adjust=False).mean().ewm(span=3, adjust=False).mean()
    ...
    day_change = (close.iloc[-1] - close.iloc[-2])/close.iloc[-2] if len(close)>=2 else 0
...
def get_stock_data(symbol, name, market_position_ratio, mode):
    try:
        ...
        # 改用yf.download 更稳定，无报错
        df = yf.download(symbol, period=f"{HIST_DAYS}d", progress=False, timeout=10)
        if len(df) < 40: 
        ...
        logger.warning(f"{symbol} 获取数据失败: {str(e)}")
        return None
