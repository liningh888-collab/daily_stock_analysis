...
# 智能双模式
NORMAL_MODE = {
    "win_loss_ratio_min": 2.0,
    "day_change_min": -0.005,
...
WEAK_MARKET_MODE = {
    "win_loss_ratio_min": 1.7,
    "day_change_min": -0.015,
...
INDUSTRY_PE_RULES = {
    ...
    "石油天然气": {"pe_max": 60, "pb_max": 4.0},  # 洲际油气54倍PE合理
    "钢铁": {"pe_max": 25, "pb_max": 2.0},
    "有色": {"pe_max": 40, "pb_max": 3.5},
    "化工": {"pe_max": 30, "pb_max": 3.0},
    
    # 消费医药：中等PE
    "医药生物": {"pe_max": 45, "pb_max": 5.0},
    "食品饮料": {"pe_max": 40, "pb_max": 6.0},
    "零售": {"pe_max": 30, "pb_max": 3.0},
    
    # 科技军工：高PE合理
    "计算机": {"pe_max": 60, "pb_max": 6.0},
    "电子": {"pe_max": 50, "pb_max": 5.0},
    "国防军工": {"pe_max": 70, "pb_max": 5.0},
...
def calc_technical_indicators(df, mode):
    ...
    is_intraday_strong = day_change >= mode["day_change_min"] and day_change <= 0.04

    # 趋势判断
    if mode["trend_up_required"]:
        trend_up = close.iloc[-1] > ma20.iloc[-1] and close.iloc[-1] > ma60.iloc[-1]
    else:
        trend_up = close.iloc[-1] > ma60.iloc[-1]
...
def get_stock_data(symbol, name, pool_type, market_position_ratio, mode):
    try:
        ...
        if not fundamental["fund_pass"]:
            logger.info(f"❌ {symbol} 基本面不达标（行业：{fundamental['industry']}，PE：{fundamental['pe']}）")
            return None
...
📊 基本面：
PE：{s['fund']['pe']}（行业上限：{INDUSTRY_PE_RULES[s['fund']['industry']]['pe_max']}）
